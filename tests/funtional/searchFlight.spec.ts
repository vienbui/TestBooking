...
test.describe('Search Flight', () => {
    ...
    for (const { departing, returning } of PERIOD_1_YEAR_SELECT_RANGE) {
        test(`[BS-004] Verify system validates with period is exactly 1 year - ${departing} to ${returning}`, async ({
            homePage,
        }) => {
    ...
    for (const { departing, returning } of PERIOD_1_5_YEAR_SELECT_RANGE) {
        test(`[BS-005] Verify system validates with period is 1.5 years - ${departing} to ${returning}`, async ({
            homePage,
        }) => {
            await homePage.searchFormComponent.searchFlight(departing, returning);
            await homePage.searchResultComponent.verifySearchResultTitleIsVisible();
    ...
    for (const { departing, returning } of PERIOD_2_YEAR_SELECT_RANGE) {
        test(`[BS-006] Verify system validates with period is 2 years - ${departing} to ${returning}`, async ({
            homePage,
        }) => {
            await homePage.searchFormComponent.searchFlight(departing, returning);
            await homePage.searchResultComponent.verifySearchResultTitleIsVisible();
    ...
    for (const { departing, returning } of PERIOD_LESS_THAN_1_YEAR_SELECT_RANGE) {
        test(`[BS-008]Verify system validates with period is less than 1 year - ${departing} to ${returning}`, async ({
            homePage,
        }) => {
            await homePage.searchFormComponent.searchFlight(departing, returning);
            await homePage.searchResultComponent.verifySearchResultTitleIsVisible();
...
test.describe('Promotional Code', () => {
    ...
    test(`[PC-008] Verify system validates with invalid promotional code with special characters`, async ({
        homePage,
    }) => {
